...
/**
 * Database-backed repository for tenant-scoped settings.
 */
class DbTenantSettingsRepository implements SettingRepositoryInterface
{
    /**
    ...
    {
        $setting = Setting::tenant($this->tenantId)
            ->byKey($key)
            ->first();
    ...
    {
        return Setting::tenant($this->tenantId)
            ->byKey($key)
            ->exists();
    }
